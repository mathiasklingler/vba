...
Sub CombineTables()
    ...
    lastRowFinal = wsFinal.Cells(wsFinal.Rows.Count, "A").End(xlUp).Row
    ...
    lastRowE = wsE.Cells(wsE.Rows.Count, "A").End(xlUp).Row
    lastRowF = wsF.Cells(wsF.Rows.Count, "A").End(xlUp).Row
    lastRowG = wsG.Cells(wsG.Rows.Count, "A").End(xlUp).Row
    lastRowH = wsH.Cells(wsH.Rows.Count, "A").End(xlUp).Row
    ...
    Set headersH = wsB.Range("A1").Resize(1, wsH.Cells(1, wsH.Columns.Count).End(xlToLeft).Column)
    ...
    targetRow = newSheet.Cells(newSheet.Rows.Count, "C").End(xlUp).Row + 1
    ...
    For Each headerCellB In headersB
        For Each headerCellFinal In headersFinal
            If headerCellB.Value = headerCellFinal.Value Then
                ...
                ' Copy data from sheet B to corresponding column in sheet Finaleliste
